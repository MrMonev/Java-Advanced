...
public class StringMatrixRotation_6 {
    ...
    private static char[][] rotate90(char[][] matrix, int rows, int cols) {
        char[][] rotateMatrix = new char[cols][rows];

        for (int col = 0; col < cols; col++) {
            int c = 0;
            for (int row = rows - 1; row >= 0; row--) {
                rotateMatrix[col][c++] = matrix[row][col];
            }
        }
        return rotateMatrix;
    }

    private static void getFillMatrix(List<String> words, char[][] matrix) {
        for (int row = 0; row < matrix.length; row++) {
